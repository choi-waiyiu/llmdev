...
def index():
    ...
    # ボットのレスポンスを取得（メモリに保持）
    get_bot_response(system_message, user_message, memory, session['thread_id'])

    # メモリからメッセージの取得
    messages = get_messages_list(memory, session['thread_id'])

    # レスポンスを返す
    return make_response(render_template('index.html', messages=messages, system_message=system_message))
